...
barplot_with_numbers <- function(df, title=''){
  ...
  df_renamed <- df %>% mutate(pathway = str_remove(pathway, "HALLMARK_")) %>%
  ...
  .genesets  <- df %>%
    filter(padj < .05) %>%
    head(40) # max cutoff
  toplot <- .genesets$pathway %>% unique()

  sel <- df %>%
    filter(pathway %in% toplot) %>%
    arrange(-abs(NES)) %>%
    arrange(-NES) %>%
  ...
  ggplot2::ggplot(
    ...
    geom_text(
      aes(label = leadingEdgeFrac,
          x = sign(NES)*.4, ),
          color='white',
      fontface = "bold",
      size=2.4,
     # position = position_dodge(width = 0.8),
     vjust = 0.5, hjust = 0.5) + theme_bw()
