...
rankobjs <- ranks_dfs_to_lists(list(data))
rankobj <- rankobjs[[1]]

res <- rankobj %>% run_one(geneset_list)
.name <- res[1, 'pathway']
...
with(enplot_data, ggplot(data = curve) + geom_line(aes(x = rank, y = ES),
    color = "green") + geom_segment(data = ticks, mapping = aes(x = rank,
...
with(enplot_data, ggplot(data = ticks) + geom_line(aes(x = rank, y = stat),
    color = "green") + geom_segment(data = ticks, mapping = aes(x = rank,
    y = -spreadES, xend = rank, yend = spreadES), linewidth = ticksSize) +
    geom_hline(yintercept = posES, colour = "red", linetype = "dashed") +
    geom_hline(yintercept = negES, colour = "red", linetype = "dashed") +
    geom_hline(yintercept = 0, colour = "black") + theme(panel.background = element_blank(),
    panel.grid.major = element_line(color = "grey92")) +
...
rankorder_edge %>% ggplot(aes(x=stat_tick, y=ES)) + geom_point()
