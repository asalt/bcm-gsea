...
if (!is.null(rankfiledir) && file.exists(rankfiledir)) {
  if (ranks_from == "volcano") {
    if (is.null(volcanodir) || !file.exists(volcanodir)){
      log_msg(msg='improper volcanodir specification')
...
```{r}
results_list[[1]][[1]] %>% dplyr::select(-c("leadingEdge")) %>% arrange(pval) %>% head(30) %>% reactable(
    resizable = T,
    wrap=F,
    bordered=T,
    filterable = F,
...
```{r, label='plot all gsea level heatmaps', fig.height=9 }
