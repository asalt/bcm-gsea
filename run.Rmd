...
```{r}
#.pca_res %>% plot( choix = "var")
.pca_res %>% factoextra::fviz_pca_ind(
  labelsize = 3.0,
  #col.ind="cos2",
  #col.ind.lab="cos2",
  select.ind = list(contrib = 10),
  habillage = colnames(.df),
  #ggp = grid::gpar(labelsize=8)
)# + ggplot2::theme(label.size=8)
...
# Example using PCA data
pca_result <- prcomp(.df, scale. = TRUE)
pca_data <- as.data.frame(pca_result$x)

# Applying K-means clustering using bluster
cluster_results <- clusterRows(pca_data[, 1:2], NNGraphParam(k = 10))
pca_data$cluster <- factor(cluster_results)

ggplot(pca_data, aes(x = PC1, y = PC2, color = cluster)) +
    geom_point() +
    theme_minimal() +
    labs(title = "PCA with Clustering", x = "PC1", y = "PC2")
```
### bluster
```{r}
...
hclust.out <- clusterRows(.df, HclustParam())
