...
params:
  ...
  gct_path: "/mnt/e/MSPC000769/results/MSPC000769_prof_AM/export/MSPC000769_prof_AM_data_gct_linear_human_1.0more_nonzero_parametricBatch_plex_noCov_normtype_median_n99x8061.gct"
  cut_by: none
...
names(rnkdfs) <- paste0("test", 1:length(datas))
...
if (!is.null(rankfiledir) && length(fs::dir_ls(rankfiledir, fail = FALSE)) == 0) {
...
plts <- results_list %>%
  plot_tools$process_results_across_rnks(
    save_func = util_tools$make_partial(save_func, width = 14, height=22)
  )
...
```{r, label='plot one gsea level heatmap', fig.height=7}
...
```{r, label='plot all gsea level heatmaps', fig.height=9, label="heatmaps"}

# here is where changes to the save func are made (e.g. width, height, file format)
.gsea_heatmap_save_func <- util_tools$make_partial(save_func, width = 24, height=18)
