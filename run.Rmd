...
results_list <- fgsea_tools$run_all_pathways(pathways_list_of_lists, ranks_list, parallel=T) # parallel if you c
...
all_res <- plot_tools$concat_results_all_collections(results_list)
.metadata <- gct@cdesc
plt <- all_res$H_ %>% plot_tools$plot_results_one_collection(., metadata = .metadata, cut_by = 'group')
```


```{r, fig.width=10, fig.height=12}
.metadata <- gct@cdesc
all_res$`C5_GO:BP` %>% plot_tools$plot_results_one_collection(., metadata = .metadata, cut_by = 'group')
```


```{r, fig.width=10, fig.height=12}
.metadata <- gct@cdesc
heatmaps <- all_res %>% plot_tools$plot_results_all_collections(metadata=.metadata, cut_by='group')
```
